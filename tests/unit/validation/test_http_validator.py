...
def test_http_validator_validates_explicit_flag_signal():
    ...
    result = validator.validate(action=action, obs=obs, state=State())
    assert result is not None
    assert result.level == "validated"
    assert result.kind == "flag_capture"


def test_http_validator_suppresses_login_gate_pages():
    validator = HttpEvidenceValidator()
    ...
    assert validator.validate(action=action, obs=obs, state=State()) is None
...
3. **Specialist-паттерн AWE.** Новый специалист под класс уязвимостей реализует пять фаз: (1) canary / probe injection, (2) context analysis, (3) filter / defense detection, (4) payload mutation, (5) evidence-gated verification. Отклонения от пятифазного паттерна допускаются, но обосновываются в docstring класса.
   ...
   > **Note (SqliSpecialist deviation):** SqliSpecialist (`penage/specialists/vulns/sqli.py`)
   > implements its pipeline as `baseline_timing → error_based_probes → blind_timing_probes
   > → candidate_finalization` rather than the canonical recon/payload_gen/delivery/
   > observation/validation naming. The phases conceptually map to the 5-phase model
   > (baseline+error_probes ≈ recon+payload_gen+delivery+observation, blind_timing
   > ≈ secondary observation, candidate_finalization ≈ validation/emission), but
   > trace event names (`sqli.phase_1.baseline_timing`, etc.) reflect the SQL-specific
   > taxonomy. Other specialists (xss, ssti, idor) use canonical phase names.
...
4. **Validation gate.** Ни одна находка не попадает в summary как `solved` без прохождения валидации. Для XSS валидация = браузерное исполнение. Для SQLi = извлечение данных или детерминированный timing-сигнал. Для SSRF = подтверждённый исходящий запрос. Для IDOR = дифференциальный тест. Теоретические находки записываются как `candidate`, не `solved`.
   ...
   > **Note (SQLi validation depth):** A `kind="sqli_finding"` action with
   > `verified=True` requires **extraction-based confirmation** (data extracted via
   > UNION SELECT, boolean inference, or stacked queries) OR **blind-timing
   > confirmation** (≥`timing_hits_required` SLEEP probes hit `timing_threshold_s`
   > delta). Backend fingerprint match alone (e.g., MySQL error string detection)
   > emits a finding with `verified=False, kind="sqli_error_fingerprint"` — this is
   > *evidence of SQL injection presence* but does NOT satisfy the verified bar.
   > E2E tests gating on `verified is True` are correct per invariant 4; relaxing
   > to fingerprint-only conflates detection with validation.
   > See `docs/roadmap/stage5.md` §1.3 for extraction enrichment plan.
...
- **Specialist vs LLM rank arbitration at `actions_per_step=1`**: at default
  `actions_per_step=1`, `_select_diverse` (`penage/policy/selection.py:85`)
  returns `ranked[:1]` (top score only). Specialist NOTE actions score
  5.0 (unverified) or 11.0 (verified); LLM coordinator actions baseline 24.0.
  Net effect: specialist findings, even verified, may never commit because they
  lose rank competition. Observed in δ.β.2.b.iii audit (SQLi E2E xfail).
  Mitigation tracked in stage 5 backlog (policy score rebalancing, k=1 seeding
  for specialist slot reservation, or default `actions_per_step≥2`);
  see `docs/roadmap/stage5.md` §1.1–1.2.
